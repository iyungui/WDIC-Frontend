    ...
    private var quoteToday: some View {
        HStack {
            VStack(alignment: .leading, spacing: 10) {
                Text("你好。\(userViewModel.user?.name ?? "")。")
    ...
    private var cardContent: some View {
        HStack {
            ...
            Button(action: {
                ...
                Image("playButton").resizable().scaledToFit().frame(width: 35, height: 35)
            }
        }
    ...
    private func cardView(item: CarouselItem) -> some View {
        ZStack {
            ...
            RoundedRectangle(cornerRadius: 20)
                .fill(item.index == currentIndex ? Color.Color11 : Color.Color12)
                ...
                HStack {
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Chapter \(item.index + 1)")
                            ...
                            .foregroundColor(item.index == currentIndex ? .black: .white)
        ...
        HStack(spacing: 8) {
            ForEach(0..<max, id: \.self) { i in
                RoundedRectangle(cornerRadius: 4)
                    .fill(i == index ? Color.Color13 : Color.Color8)
